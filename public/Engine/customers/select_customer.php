...
{
	$pos_customer_id = $_POST['radio'];	
	header('Location: ' . POS_ENGINE_URL .'/office/address/address.php?ref='.$complete_location .'&type=add&pos_customer_id='.$pos_customer_id);
...
$html .= '<p>';
$html .= '<input class = "button" type="button" style="width:300px" name="add_customer" value="Add Customer" onclick="open_win(\'add_edit_customer.php?type=ADD_SELECT&complete_location='.$complete_location.'\')"/>';

$html .= '</p>';
//now make the table
$html .= createSearchFormWithID($search_fields,$action, array('complete_location' => $complete_location));
