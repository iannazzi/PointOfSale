<?php 

/*
	The process to create a new db entry for things like mfg, employee, etc is to define what we want in the table, then create that table....event post the table in json format for the handler to process?
	
	Craig Iannazzi 1-23-12
	
*/
$binder_name = 'Manufacturers';
$access_type = 'WRITE';
require_once ('../manufacturer_functions.php');

$complete_location = '../ViewManufacturer/view_manufacturer.php';
$cancel_location = '../ListManufacturers/list_manufacturers.php?message=Canceled';

$db_table = 'pos_manufacturers';
$data_table_def = array( 
						array( 'db_field' => 'company',
								'type' => 'input',
								'validate' => array('unique' => 'true', 'min_length' => 1),
								'db_table' => $db_table),
						/*array( 'db_field' => 'pos_account_id',
								'type' => 'select',
								'caption' => 'Account ID',
								'html' => createInventoryAccountSelect('pos_account_id', 'false'),
								'validate' => 'none'),*/
						array('db_field' => 'sales_rep',
								'caption' => 'Sales Representative(s) (commas to separate names)',
								'type' => 'input',
								'validate' => 'none'),
						array('db_field' =>  'email',
								'type' => 'input',
								'validate' => 'none'),
						array('db_field' => 'phone',
								'type' => 'input',
								'tags' => 'maxlength = "40" ',
								'validate' => 'none'),
						array('db_field' =>  'fax',
								'type' => 'input',
								'tags' => 'maxlength = "40" ',
								'validate' => 'none'),
						array('db_field' => 'address1',
								'type' => 'input',
								'validate' => 'none'),
						array('db_field' => 'address2',
								'type' => 'input',
								'validate' => 'none'),
						array('db_field' => 'city',
								'type' => 'input',
								'validate' => 'none'),
						array('db_field' => 'state',
								'type' => 'input',
								'validate' => 'none'),
						array('db_field' => 'zip',
								'type' => 'input',
								'tags' => 'maxlength = "20" ',
								'validate' => 'none'),
						array('db_field' => 'country',
								'type' => 'input',
								'validate' => 'none'),
						array('db_field' =>  'active',
								'type' => 'checkbox',
								'tags' => 'checked="checked" ',
								'validate' => 'none'),
						array('db_field' => 'comments',
								'type' => 'textarea',
								'validate' => 'none'));
//Header
include (HEADER_FILE);
//set up form
//Manufacturer Table
$html = '<h2>Add A Manufacturer/Supplier</h2>
		<p>This is the Parent Company which will have at least one brand<br>
				(manufacturers and brands often have different names - this lets you relate manufacturers to brands. </p>';
$form_handler = 'add_manufacturer.form.handler.php';
$html .= createTableForMYSQLInsert($data_table_def, $form_handler, $complete_location, $cancel_location);
//footer
echo $html;
include (FOOTER_FILE);

?>

