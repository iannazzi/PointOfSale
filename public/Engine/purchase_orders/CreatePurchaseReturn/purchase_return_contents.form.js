	...
	if (json_tbody_data.length > 0)
	{
		for(var row=0;row<json_tbody_data.length;row++)
		{
			...
			for(var col=1;col<json_tbody_data[0].length-1;col++)
			{
				...
				//alert("temp Name: " + tmpName + " new value: " + tbody_data[row][col] );
...
function deleteRow(tableID) 
{
	...
	{	// delete selected rows
		...
		{
			//first find the row(s) that are checked
			checked_rows = findCheckedRows(tableID);
			if (checked_rows.length>0)
			{
				//next copy the entire table to an array - need the size rows as well
				tableData = copyTableDataToArray(tableID);
				//next add the appropriate number of rows
				for (var i=0;i<checked_rows.length;i++)
				{
				...
				writeArrayToTable(newData, tableID);
				updateRowEditabilityLinksTotalAndNames();
				setRowFocus(checked_rows[0]);	
			}
			else
...
function loadDataFromStyleNumber(control)
{
	...
	stlyeAjax[test_row] = $.getJSON("get_style_data.php", 
		{ style_number: style_number_to_check, pos_manufacturer_id: pos_manufacturer_id, pos_manufacturer_brand_id: pos_manufacturer_brand_id, datehack: dateHack  },
		function(data)
		{
  			if (data != null)
  			 {
				...
				if (data[0].source == 'pos_system')
				{
					...
					setSizeChartBasedOnCategory(control);
				...
				else
				{
					...
					sizes_for_current_row[test_row] = 'undefined';
					highlightSizes(sizes_for_current_row[test_row]);
					findLinkedRow(control);
				}
  			 }
...
function setRowFocus(row)
{
	...
	document.getElementById('poc_tbody').rows[row].cells[style_col].childNodes[0].focus();
	current_row = row;
}
