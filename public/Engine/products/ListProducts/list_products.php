...
$search_fields = array(		array(	'db_field' => 'pos_product_id',
											'mysql_search_result' => 'pos_product_id',
											'caption' => 'System Id ',	
											'type' => 'input',
											'html' => createSearchInput('pos_product_id')),
							array(	'db_field' => 'pos_category_id',
											'mysql_search_result' => 'pos_category_id',
...
SELECT 	pos_products.pos_product_id,  
		...
		(SELECT GROUP_CONCAT( DISTINCT CONVERT( pos_purchase_orders.pos_purchase_order_id, char( 10 ) ) ) 
			...
			WHERE pos_products.pos_product_id = pos_products_sub_id.pos_product_id) as purchase_orders
...
$html .= '<p>';
//$html .= '<input class = "button" type="button" style="width:180px;" name="add_product" value="List Sub Ids" onclick="open_win(\'list_product_sub_ids.php\')"/>';
$html .= createUserButton('Product Sub Ids');
if(checkWriteAccess($binder_name))
{
	...
	$html .= createUserButton('Product Attributes');
	$html .= '<input class = "button" type="button" style="width:180px;" name="bulk_product" value="Bulk Create Products" onclick="open_win(\'../BulkProductGeneration/bulk_product_generation.php\')"/>';
	
	$html .= '<input class = "button" type="button" style="width:180px;" name="select_products" value="Select Products" onclick="open_win(\'../selectProducts/select_products.php\')"/>';
	$html .= '<input class = "button" type="button" style="width:180px;" name="select_products" value="Bulk Create Products and Inventory" onclick="open_win(\'../selectProducts/select_products.php\')"/>';
	$html .= '</p>';
	$html .= '<p>';
...
$html .= '</p>';
$html .= '<p>';
$html .= '<form  name="frmLoadImages" action="../ProductImages/product_image_coordinator.php"  method="post">';

 $html .= '<input class = "button" type="submit" style="width:220px;" name="image_coordination" value="Take Photos Of These Products" />';
$html .= createHiddenInput('tmp_sql', urlencode($tmp_sql));
$html .= createHiddenInput('tmp_select_sql', urlencode($tmp_select_sql));
$html .= createHiddenInput('order_sql', urlencode($order_sql));
$html .= createHiddenInput('search_sql', urlencode($search_sql));
