...
//function createOperatingExpenseTitle()
	...
	<tbody class = "title_body">
		<tr><th class = "title" colspan ="5">OPERATING EXPENSES</th></tr>
	</tbody>';
...
//function createOperatingExpenseTable($year, $store_id)
	...
	for($category = 0;$category < sizeof($expense_categories); $category++)
	{
		...
		for($month = 1; $month<13; $month++)
		{
			//these are payments with no journal entry, we have to get to the chart of accounts by looking at the account id, so if an account is not linked to a chart of account then it will not get it....
			$payment_sql = "SELECT sum(payment_amount) 
							FROM pos_payments_journal 
							LEFT JOIN pos_accounts ON pos_payments_journal.pos_payee_account_id = pos_accounts.pos_account_id
							WHERE pos_payments_journal_id NOT IN (SELECT pos_payments_journal_id FROM pos_invoice_to_payment WHERE pos_payments_journal_id = pos_payments_journal.pos_payments_journal_id) 
							
							AND month(payment_date) = '$month' 
							AND year(payment_date) = '$year'
							AND (SELECT pos_chart_of_accounts_id FROM pos_accounts_to_chart_of_accounts WHERE pos_accounts_to_chart_of_accounts.pos_account_id = pos_accounts.pos_account_id LIMIT 1) = " . $expense_categories[$category]['pos_chart_of_accounts_id'];
							
			//this gets the payment date, and finds the journal entry to get the chart of accounts id
			//basically we are doing this backwards.... we want the journal information?
			
			$payment_to_general_journal_sql = "SELECT sum(payment_amount) 
							FROM pos_payments_journal 
							WHERE month(pos_payments_journal.payment_date) = '$month' 
							AND year(pos_payments_journal.payment_date) = '$year'
							AND (SELECT pos_general_journal.pos_chart_of_accounts_id FROM pos_general_journal
									LEFT JOIN pos_invoice_to_payment ON pos_general_journal.pos_general_journal_id = pos_invoice_to_payment.pos_journal_id 
									WHERE pos_invoice_to_payment.source_journal ='GENERAL JOURNAL' AND pos_invoice_to_payment.pos_payments_journal_id = pos_payments_journal.pos_payments_journal_id LIMIT 1) = " .$expense_categories[$category]['pos_chart_of_accounts_id'];
							
			
			
			//this does not return the 'payment date' so do not use it..
			$expense_category_month_year_sql = "SELECT sum(entry_amount) FROM pos_general_journal 
									WHERE month(invoice_date) = '$month' 
									AND year(invoice_date) = '$year'
									AND pos_chart_of_accounts_id = " . $expense_categories[$category]['pos_chart_of_accounts_id'];
			if ($store_id != 'all')
			{
				$expense_category_month_year_sql = $expense_category_month_year_sql . " AND (SELECT pos_general_journal.pos_store_id FROM pos_general_journal
									LEFT JOIN pos_invoice_to_payment ON pos_general_journal.pos_general_journal_id = pos_invoice_to_payment.pos_journal_id 
									WHERE pos_invoice_to_payment.source_journal ='GENERAL JOURNAL' AND pos_invoice_to_payment.pos_payments_journal_id = pos_payments_journal.pos_payments_journal_id LIMIT 1) = '" . $store_id ."'";
				$payment_to_general_journal_sql = $payment_to_general_journal_sql . " AND (SELECT pos_general_journal.pos_store_id FROM pos_general_journal
									LEFT JOIN pos_invoice_to_payment ON pos_general_journal.pos_general_journal_id = pos_invoice_to_payment.pos_journal_id 
									WHERE pos_invoice_to_payment.source_journal ='GENERAL JOURNAL' AND pos_invoice_to_payment.pos_payments_journal_id = pos_payments_journal.pos_payments_journal_id LIMIT 1) = '" . $store_id ."'";

				$payment_sql = $payment_sql . " AND pos_payments_journal.pos_store_id = '" . $store_id ."'";
			}
			//$html .= '<td>' . round(getSingleValueSQL($expense_category_month_year_sql),0) . '</td>';
			//$html .= '<td>' . round(getSingleValueSQL($payment_sql),0) . '</td>';
			$html .= '<td>' . round(getSingleValueSQL($payment_to_general_journal_sql)+getSingleValueSQL($payment_sql),0) . '</td>';
		...
		$payment_to_general_journal_sql = "SELECT sum(payment_amount) 
							FROM pos_payments_journal 
							WHERE year(pos_payments_journal.payment_date) = '$year'
							AND (SELECT pos_general_journal.pos_chart_of_accounts_id FROM pos_general_journal
									LEFT JOIN pos_invoice_to_payment ON pos_general_journal.pos_general_journal_id = pos_invoice_to_payment.pos_journal_id 
									WHERE pos_invoice_to_payment.source_journal ='GENERAL JOURNAL' AND pos_invoice_to_payment.pos_payments_journal_id = pos_payments_journal.pos_payments_journal_id LIMIT 1) = " .$expense_categories[$category]['pos_chart_of_accounts_id'];
		...
		$expense_category_year_sql = "SELECT sum(entry_amount) FROM pos_general_journal 
									WHERE year(invoice_date) = '$year'
									AND pos_chart_of_accounts_id = " . $expense_categories[$category]['pos_chart_of_accounts_id'];
		if ($store_id != 'all')
			{
				$expense_category_year_sql = $expense_category_year_sql . " AND (SELECT pos_general_journal.pos_store_id FROM pos_general_journal
									LEFT JOIN pos_invoice_to_payment ON pos_general_journal.pos_general_journal_id = pos_invoice_to_payment.pos_journal_id 
									WHERE pos_invoice_to_payment.source_journal ='GENERAL JOURNAL' AND pos_invoice_to_payment.pos_payments_journal_id = pos_payments_journal.pos_payments_journal_id LIMIT 1) = '" . $store_id ."'";
				$payment_sql = $payment_sql . " AND pos_payments_journal.pos_store_id = '" . $store_id ."'";
				$payment_to_general_journal_sql = $payment_to_general_journal_sql . " AND (SELECT pos_general_journal.pos_store_id FROM pos_general_journal
		...
		//$html .= '<td>' . round(getSingleValueSQL($expense_category_year_sql),0) . '</td>';
		//$html .= '<td>' . round(getSingleValueSQL($expense_category_year_sql),0) . '</td>';
		$html .= '<td>' . number_format(getSingleValueSQL($payment_to_general_journal_sql)+getSingleValueSQL($payment_sql),0) . '</td>';
		$html .= '</tr>';
...
//function createOperatingExpenseFooter($year, $store_id)
	...
	<tbody class = "income_footer">
		...
		for($month = 1; $month<13; $month++)
		{
			...
			//this is looking for a payment to an expense account that is not linked to an invoice.... (9 is an expense account type....
			//basically this looks incorrect. An invoice in the general journal should eventually be paid....
			//lets say we have a bunch of stuff racked up on mayer hardware and we send a payment....
			//send a payment.... how do we properly expense it....
			//not this way....
			...
			$payment_sql = "SELECT sum(payment_amount) 
							...
							AND year(payment_date) = '$year'
							AND
							(SELECT pos_chart_of_accounts.active FROM pos_chart_of_accounts
							LEFT JOIN pos_accounts_to_chart_of_accounts ON pos_accounts_to_chart_of_accounts.pos_chart_of_accounts_id = pos_chart_of_accounts.pos_chart_of_accounts_id
							WHERE pos_accounts_to_chart_of_accounts.pos_account_id = pos_payments_journal.pos_payee_account_id LIMIT 1) = 1
							AND
							(SELECT pos_chart_of_accounts.pos_chart_of_account_type_id FROM pos_chart_of_accounts
							LEFT JOIN pos_accounts_to_chart_of_accounts ON pos_accounts_to_chart_of_accounts.pos_chart_of_accounts_id = pos_chart_of_accounts.pos_chart_of_accounts_id
							WHERE pos_accounts_to_chart_of_accounts.pos_account_id = pos_payments_journal.pos_payee_account_id LIMIT 1) = 9
";							
			//this gets the payment date, and finds the journal entry to get the chart of accounts id
			$payment_to_general_journal_sql = "SELECT sum(payment_amount) 
							FROM pos_payments_journal 
							WHERE month(pos_payments_journal.payment_date) = '$month' 
							AND year(pos_payments_journal.payment_date) = '$year'
							AND (SELECT pos_chart_of_accounts.active
								FROM pos_chart_of_accounts 
								LEFT JOIN pos_general_journal ON pos_general_journal.pos_chart_of_accounts_id = pos_chart_of_accounts.pos_chart_of_accounts_id
									LEFT JOIN pos_invoice_to_payment ON pos_general_journal.pos_general_journal_id = pos_invoice_to_payment.pos_journal_id 
									WHERE pos_invoice_to_payment.source_journal ='GENERAL JOURNAL' AND pos_invoice_to_payment.pos_payments_journal_id = pos_payments_journal.pos_payments_journal_id LIMIT 1) = 1
									
							AND (SELECT pos_chart_of_accounts.pos_chart_of_account_type_id
								FROM pos_chart_of_accounts 
								LEFT JOIN pos_general_journal ON pos_general_journal.pos_chart_of_accounts_id = pos_chart_of_accounts.pos_chart_of_accounts_id
									LEFT JOIN pos_invoice_to_payment ON pos_general_journal.pos_general_journal_id = pos_invoice_to_payment.pos_journal_id 
									WHERE pos_invoice_to_payment.source_journal ='GENERAL JOURNAL' AND pos_invoice_to_payment.pos_payments_journal_id = pos_payments_journal.pos_payments_journal_id LIMIT 1) = 9
									
									";
		
		
		
			$expense_month_year_sql = "SELECT sum(entry_amount) FROM pos_general_journal 
			LEFT JOIN  pos_chart_of_accounts ON pos_general_journal.pos_chart_of_accounts_id = pos_chart_of_accounts.pos_chart_of_accounts_id 
									WHERE month(invoice_date) = '$month' 
									AND year(invoice_date) = '$year'
									AND pos_chart_of_accounts.active = 1 AND pos_chart_of_accounts.pos_chart_of_account_type_id = 9";
			if ($store_id != 'all')
			{
				$expense_month_year_sql = $expense_month_year_sql . " AND pos_general_journal.(SELECT pos_general_journal.pos_store_id FROM pos_general_journal
									LEFT JOIN pos_invoice_to_payment ON pos_general_journal.pos_general_journal_id = pos_invoice_to_payment.pos_journal_id 
									WHERE pos_invoice_to_payment.source_journal ='GENERAL JOURNAL' AND pos_invoice_to_payment.pos_payments_journal_id = pos_payments_journal.pos_payments_journal_id LIMIT 1) = '" . $store_id ."'";
				$payment_to_general_journal_sql = $payment_to_general_journal_sql . " AND (SELECT pos_general_journal.pos_store_id FROM pos_general_journal
									LEFT JOIN pos_invoice_to_payment ON pos_general_journal.pos_general_journal_id = pos_invoice_to_payment.pos_journal_id 
			...
			$expense[$month] = getSingleValueSQL($payment_sql)+getSingleValueSQL($payment_to_general_journal_sql);
			$expense_total = $expense_total + $expense[$month];
			$html .= '<td>' . number_format($expense[$month],0) . '</td>';
...
";							
		...
		$expense_category_year_sql = "SELECT sum(entry_amount) FROM pos_general_journal 
		LEFT JOIN  pos_chart_of_accounts ON pos_general_journal.pos_chart_of_accounts_id = pos_chart_of_accounts.pos_chart_of_accounts_id 
									WHERE year(invoice_date) = '$year' AND pos_chart_of_accounts.active = 1 AND pos_chart_of_accounts.pos_chart_of_account_type_id = 9";
		if ($store_id != 'all')
			{
				$expense_category_year_sql = $expense_category_year_sql . " AND pos_store_id = '" . $store_id ."'";
				$payment_to_general_journal_sql = $payment_to_general_journal_sql . " AND pos_store_id = '" . $store_id ."'";
				$payment_sql = $payment_sql . " AND pos_store_id = '" . $store_id ."'";
			}
			$html .= '<td>'  . number_format($expense_total,0) . '</td>';
		$html .= '</tr>';
...
for($month = 1; $month<13; $month++)
	{
			$net_income[$month] = $gross_sales_month[$month] + $returns_month[$month] - $cogs_month[$month] - $expense[$month];
			$html .= '<td>'  . number_format($net_income[$month] ,0). '</td>';
	}
	$net_income_total = $gross_sales_total + $returns_total - $cogs_total - $expense_total;
