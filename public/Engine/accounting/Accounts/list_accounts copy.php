...
SELECT pos_accounts.*, pos_account_type.account_type_name, pos_chart_of_accounts.account_number  AS chart_account_number, pos_chart_of_accounts.account_name ,
		...
		COALESCE((SELECT pos_account_balances.balance_amount
		FROM pos_account_balances
		WHERE pos_account_balances.pos_account_id = pos_accounts.pos_account_id 
		AND pos_account_balances.balance_date = (
		SELECT MAX( pos_account_balances.balance_date )
		FROM pos_account_balances
		WHERE pos_account_balances.pos_account_id = pos_accounts.pos_account_id)),0)
		
		as opening_balance,
		
		
		
		COALESCE((SELECT sum(entry_amount-discount_applied) FROM pos_general_journal WHERE pos_account_id = pos_accounts.pos_account_id AND entry_type = 'Invoice' AND DATE(entry_date)>=ifnull(DATE((SELECT MAX( pos_account_balances.balance_date )
		FROM pos_account_balances
		WHERE pos_account_balances.pos_account_id = pos_accounts.pos_account_id)),'0000-00-00')),0)
		
		as gj_invoices,
		
		(SELECT COALESCE(sum(invoice_amount-discount_applied),0) FROM pos_purchases_journal WHERE invoice_type = 'Regular' AND pos_account_id = pos_accounts.pos_account_id AND DATE(invoice_date)>=ifnull(DATE((SELECT MAX( pos_account_balances.balance_date )
		FROM pos_account_balances
		WHERE pos_account_balances.pos_account_id = pos_accounts.pos_account_id)),'0000-00-00'))
		

		as pj_invoices,
				(SELECT COALESCE(sum(invoice_amount),0) FROM pos_purchases_journal WHERE invoice_type = 'Credit Memo' AND pos_account_id = pos_accounts.pos_account_id AND DATE(invoice_date)>=ifnull(DATE((SELECT MAX( pos_account_balances.balance_date )
		FROM pos_account_balances
		WHERE pos_account_balances.pos_account_id = pos_accounts.pos_account_id)),'0000-00-00'))
		...
		(SELECT COALESCE(sum(payment_amount),0) FROM pos_payments_journal WHERE pos_account_id = pos_accounts.pos_account_id AND DATE(payment_date)>=ifnull(DATE((SELECT MAX( pos_account_balances.balance_date )
		FROM pos_account_balances
		WHERE pos_account_balances.pos_account_id = pos_accounts.pos_account_id)),'0000-00-00'))
		...
		(SELECT COALESCE(sum(payment_amount),0) FROM pos_payments_journal WHERE pos_payee_account_id = pos_accounts.pos_account_id AND DATE(payment_date)>=ifnull(DATE((SELECT MAX( pos_account_balances.balance_date )
		FROM pos_account_balances
		WHERE pos_account_balances.pos_account_id = pos_accounts.pos_account_id)),'0000-00-00'))
...
;
";
$tmp_select_sql = "SELECT *, opening_balance+gj_invoices+pj_invoices -pj_credits + payments_using - payment_to as balance FROM accounts WHERE 1";
