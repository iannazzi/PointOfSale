...
if (isset($_POST['submit'])) 
{
	...
	$payment_insert_info =  array(		'pos_store_id' => $_SESSION['store_id'],
										...
										'source_journal' => 'GENERAL JOURNAL', //this was originally marked as general
	...
	if (isset($_POST['pos_general_journal_id']))
	{
		$invoices_effected = getSQL("SELECT pos_journal_id, applied_amount FROM pos_invoice_to_payment WHERE pos_payments_journal_id = $pos_payments_journal_id AND source_journal = 'GENERAL JOURNAL'");
	//probably want to set all of those po's to incomplete
		for($i=0;$i<sizeof($invoices_effected);$i++)
		{
			if($invoices_effected[$i]['applied_amount'] != 0)
			{
				...
				$pos_general_journal_id = $invoices_effected[$i]['pos_journal_id'];				 
				 $tmp_key_val_id['pos_general_journal_id'] = $pos_general_journal_id;
				 $pay_status_update_array = array('payment_status' => 'UNPAID');
				 $invoice_status_update_array = array('invoice_status' => 'OPEN');
				 simpleTransactionUpdateSQL($dbc,'pos_general_journal', $tmp_key_val_id, $pay_status_update_array);				 	
				 simpleTransactionUpdateSQL($dbc,'pos_general_journal', $tmp_key_val_id, $invoice_status_update_array);
				 
			 }
		}
		$delete_sql = "DELETE FROM pos_invoice_to_payment WHERE pos_payments_journal_id = $pos_payments_journal_id";
		$result[] = runtransactionsql($dbc, $delete_sql);
		for($i=0;$i<sizeof($_POST['pos_general_journal_id']);$i++)
		{
			$pos_general_journal_id = $_POST['pos_general_journal_id'][$i];
			if(!in_array($_POST['pos_general_journal_id'][$i], array('NULL', '', 'false')))
			{
				$applied_amount = $_POST['applied_amount_from_this_payment'][$i];
				$po_invoice_insert_array = array('pos_journal_id' => $pos_general_journal_id,
	...
	if (isset($_POST['pos_general_journal_id']))
	{
		tryToCloseExpenseInvoices($_POST['pos_general_journal_id']);
