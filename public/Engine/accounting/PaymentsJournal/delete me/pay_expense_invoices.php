...
$selected_invoices[0]['pos_general_journal_id'] = $pos_general_journal_id;
$auto_pay_account_id = getAutoPayAccountId($pos_account_id);
$data_table_def = array( 
						array('db_field' => 'pos_general_journal_id',
								'type' => 'select',
								'caption' => 'Invoices<br>To Apply Payment To<br><br>Use Control, Shift, and/or <br>Command To Select Multiple',
								'html' => createUnpaidExpenseInvoiceSelect('pos_general_journal_id[]', $pos_account_id, $selected_invoices, 'off', ' multiple size="5" onclick="updateAmountDue()" onchange="needToConfirm=true" '),
...
include (HEADER_FILE);
$html = '<p>Pay An Invoice To '.getAccountName($pos_account_id).'</p>';
$html .='<script src="pay_expense_invoices.js"></script>'.newline();
$html .= '<script>var open_invoices = ' . json_encode($open_invoices) . ';</script>';
$form_handler = 'pay_expense_invoices.form.handler.php';
