...
function updateInvoiceData(control)
{
	//alert('running updatePOData');
	copyHTMLTableDataToArray();
	var row = getCurrentRow(control);
	pos_purchases_journal_id = control.value;
	
	//this is how we handle the pre-loaded data...
	for(var i=0;i<selectable_invoices.length;i++)
	{
		if(selectable_invoices[i]['pos_purchases_journal_id'] == pos_purchases_journal_id)
		{
			updateItemDataInTableArray(selectable_invoices[i], row);
		}
	}
...
function updateAppliedAmountRemaining(control)
{
	...
	document.getElementsByName('applied_amount_remaining[]')[row].value = round2(document.getElementsByName('invoice_amount[]')[row].value - document.getElementsByName('discount_applied[]')[row].value - document.getElementsByName('credit_memos_applied[]')[row].value - document.getElementsByName('applied_amount_from_other_payments[]')[row].value - control.value,2);
	updateTableData(control);
	
	
	//document.getElementById('payment_amount').value = round2(calculateDynamicTableColumnTotal('applied_amount_from_this_payment'),2);
...
function updateDiscount()
{
	...
	invoice_amount = goods_amount + shipping_amount;
	
	show_discount = round(invoice_amount*(show_discount_percent/100),2);
	invoice_discount = round((invoice_amount - shipping_amount - show_discount)*(discount/100),2);
	total_discount = round(show_discount + invoice_discount,2);
