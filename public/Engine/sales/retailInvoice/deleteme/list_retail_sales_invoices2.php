...
*/
$page_title = 'Sales Invoices full report';
...
$search_set = saveAndRedirectSearchFormUrl($search_fields, 'saved_sales_invoices_url2');
...
SELECT  
		...
		(SELECT sum(extension-tax_total) FROM pos_sales_invoice_contents WHERE pos_sales_invoice_contents.pos_sales_invoice_id = pos_sales_invoice.pos_sales_invoice_id
		) as ext_tax,
		
		(SELECT sum(
		if (content_type = 'CREDIT_CARD' OR content_type='SHIPPING', 
			quantity*retail_price, 
			quantity*if(discount_type='DOLLAR', 
				(sale_price-discount -applied_instore_discount), 
				( sale_price - applied_instore_discount - (sale_price*(discount/100)))
		)
		)) FROM pos_sales_invoice_contents WHERE pos_sales_invoice_contents.pos_sales_invoice_id = pos_sales_invoice.pos_sales_invoice_id) as calc_price,
		
		(SELECT sum(extension) FROM pos_sales_invoice_contents WHERE pos_sales_invoice_contents.pos_sales_invoice_id = pos_sales_invoice.pos_sales_invoice_id
		) as sum_extension,
		...
		LEFT JOIN pos_customer_payment_methods
		ON pos_customer_payments.pos_customer_payment_method_id = pos_customer_payment_methods.pos_customer_payment_method_id
		WHERE pos_sales_invoice_to_payment.pos_sales_invoice_id = pos_sales_invoice.pos_sales_invoice_id AND (pos_customer_payment_methods.payment_type = 'Discover' OR pos_customer_payment_methods.payment_type = 'Visa' OR pos_customer_payment_methods.payment_type = 'American Express' OR pos_customer_payment_methods.payment_type = 'Master Card')) as credit,
...
";
$tmp_select_sql = "SELECT *, grand_total_from_contents - grande_total as err2, total_payment - grande_total as error
